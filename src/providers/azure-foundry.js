...
class AzureFoundryProvider extends BaseProvider {
  constructor(apiKey, endpoint, model = 'claude-sonnet-4-20250514') {
  ...
  getModelInfo() {
    return { name: this.model, provider: 'azureFoundry', maxTokens: 200000 };
  }
  ...
  async *streamChat(messages, tools, options = {}) {
    try {
      ...
      logger.error('Azure Foundry (Anthropic) streaming error', err);
